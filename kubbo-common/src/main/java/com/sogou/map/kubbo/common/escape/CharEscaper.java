 ...
 * {@link com.google.common.xml.XmlEscapers}. To create your own escapers extend
 * this class and implement the {@link #escape(char)} method.
 */
...
public abstract class CharEscaper implements Escaper {
    ...
    public String escape(String string) {
        ...
        // Inlineable fast-path loop which hands off to escapeSlow() only if
        // needed
    ...
    protected final String escapeSlow(String s, int index) {
        ...
        if (charsLeft > 0) {
            ...
            if (destSize < sizeNeeded) {
                ...
                // Regrow and copy, expensive! No padding as this is the final
                // copy.
     ...
     * character does not need to be escaped. If an empty array is returned,
     * this effectively strips the input character from the resulting text.
     *
     * <p>
    ...
    private static char[] growBuffer(char[] dest, int index, int size) {
        if (size < 0) { // overflow - should be OutOfMemoryError but GWT/j2cl
                        // don't support it
